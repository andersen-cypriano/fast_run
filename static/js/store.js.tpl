...
DOMContentLoaded.addEventOrExecute(() => {
    ...
    {% set should_show_discount = should_show_discount_combination and product.maxPaymentDiscount.value > 0 %}
    {% if should_show_discount %}
                ...
                if (showMaxPaymentDiscount(variant)){
                    {% for key, method in product.payment_methods_config %}
                        {% if method.max_discount > 0 %}
                            {% if (not should_show_discount_combination or method.allows_discount_combination) %}
        ...
        function showMaxPaymentDiscountNotCombinableDisclaimer(variant) {
            {% if not should_show_discount_combination or product.maxPaymentDiscount()["allowsDiscountCombination"] %}
                return false
            {% elseif product.free_shipping %}
                return false
            {% else %}
